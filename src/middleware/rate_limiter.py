...
class RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Using time.time() + window isn't fully accurate for reset when requests are spaced out,
        # but it gives an upper bound. The proper reset time for the *first* request in the window is:
        # history[0] + window. But we don't return that on success currently.
        # Let's just use time.time() + window for simplicity if history isn't exposed here, 
        # or we could improve `check_rate_limit` to return the reset time. 
        # For MVP, `time.time() + window` is acceptable.
